...
| **SeasonIQ portal** | `frontend/season.html` | **The in-season app** (SEASON chip in DraftIQ's fbar, or /season.html): MY MATCHUP (live weekly box-score projections, win prob via league weekly σ), START/SIT (current vs optimal lineup, concrete swaps, bench-leak pts — `/api/startsit`), PROJECTED STANDINGS (title/playoff Monte Carlo re-run on LIVE rosters — `/api/season-odds`), the decision-grade wire + room activity embedded. Self-contained page, own compact styles. |
