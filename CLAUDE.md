...
| **Waiver wire (IN-SEASON)** | `models/waivers.py` | **The post-draft edge, operational** (league DNA: in-season streaming + waivers are the only repeatable owner skills). Live ESPN rosters + free agents (same auth as espn_proj), valued on **TRUE rest-of-season** = ESPN's league-scored season aggregate MINUS banked actuals (`projected_total_points − total_points`, clamped ≥0). **Backtested 2026-09-15** (2024+2025 box_players, wks 3/5/8/11, 23,855 same-position decision pairs): pure ROS ranks actual remaining points at **90.8% vs 84.5%** for the raw aggregate, and the aggregate DECAYS all season (wk-11 Spearman .66-.75 vs .85-.88) — never value in-season moves on the raw aggregate. In-season replacement = the 5TH-best FA at the position (best-FA zeroes vsWire by construction). **2026-09-15 overhaul (subscriber-review roadmap, all shipped):** decision thresholds are pts-per-remaining-week (LINEUP_CRACK_PW 0.5 / WATCH_PW 0.7 — fixed ROS constants were noise in Sept, dead by Dec); actions are slot-labeled, IR-aware, and SEQUENCED against a working roster; STREAM actions carry a claim ladder + next-week look-ahead; **waiver-PRIORITY economics** per action (my waiver_rank, teams-ahead-who'd-start-him heuristic, burn-vs-wait verdict, submitOrder — ESPN burns priority on the first list hit); market heat (roster% + daily delta archive) as display-only tiebreak; season-aware Monte Carlo (real records/schedule/decaying shock — replay-validated vs 2019-25); IDP watchlist-eligible (persistence 0.158 ≈ TE = hold asset); trade map auto-suppressed wk 5+ until the preseason "ours" board gets a weekly reprice; stale-serve + fetchedAt everywhere; one League object per 120s; daily archive snapshotter thread in api.py. Sections: UPGRADES (cracks your optimal lineup, via inseason.waiver_targets, each with a drop suggestion), DEPTH ADDS, D/ST STREAM for the current week (week1_odds is week-parameterized; implied-opp-total, the validated +55/yr play), INJURY FLAGS (OUT/IR/susp), ROOM ACTIVITY (lg.recent_activity). `/api/waivers` (5-min cache, ?week=N ?force=1), ⇄ WIRE chip in the fbar + terminal `wire`/`waivers`, CLI `python models/waivers.py`. |
| **SeasonIQ site** | `frontend/season/` | **The in-season app is its own SITE** (SEASON chip in DraftIQ, or /season/; old /season.html redirects). Shared `season.css`+`season.js` (nav, fetch, shared wire renderer), four pages: **index** (dashboard — verdict cards + one-glance verdicts), **matchup** (both full lineups w/ ⚕/sit flags, start/sit block, win prob — `/api/startsit` now includes the opponent's lineup), **wire** (decision-grade report + room activity), **standings** (dual-judge Monte-Carlo table ESPN-vs-our-model, TRADE MAP from judge disagreements, title% trajectory canvas). `season_odds` appends ONE snapshot/day to `data/processed/season_history.jsonl` (served by `/api/season-history`) — the archive starts before it's needed. **TRADES** page (`/api/rosters` both-judge roster values + counterparty dossier from manager_profiles + logged moves; `/api/trade2` dual-judge BOTH-sides eval — verdict logic: propose when it helps you by OUR numbers and looks fine to them on ESPN's). **RESULTS** page (`/api/performance` — W/L, actual-vs-proj, hindsight bench leak per completed week + live league table; auto-fills from week 1). ⚠ **League is waiver-PRIORITY, NOT FAAB** (settings.faab=False — draft-era coach prompts said FAAB; corrected in season copy). Vanilla + zero build, deliberately — DraftIQ stays frozen as the draft-day artifact. |
| **Weekly ML model** | `models/weekly_model.py` | **Bake-off with an honest gate**: predict player-week points, benchmark = ESPN's own weekly projection, train 2019-23 / test 2024-25 (box_players.csv, 16.8k player-weeks). **v1 verdict: DO NOT SHIP** — lags+bias features lose to the vendor on all 3 metrics (MAE 5.03 vs 4.99, r .503 vs .514, coin-flip decision acc 54.6% vs 55.7%). Eval card: data/processed/weekly_model.json. v2 needs vendor-invisible features: opp-vs-position strength, Vegas lines, snap/target shares. The portal deliberately runs on ESPN weekly projections until a model clears the gate. |
...
| **NFL schedule** | `models/nfl_schedule.py` | Full 18-week NFL schedule from ESPN's free scoreboard API (cached 7d, WSH/JAC→WAS/JAX normalized). Powers `/api/planner`: bye-crunch forecast (next 4 fantasy weeks, alert at 3+ starters out) + weeks-15-17 playoff-opponent panel on the standings page. Known 2026 landmine: **week 11 has BOTH QBs (Maye/NE, Love/GB) on bye** — the crunch alert will fire at week 8. |
