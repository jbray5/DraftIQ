...
| Piece | Path | Notes |
|---|---|---|
| Flask API | `api.py` | `/api/board` (DraftIQ value board), `/api/ai/best-pick` (**fused Optimal-Pick coach — primary on-the-clock**), `/api/ai/opinion` (Claude deep-dive), `/api/ai/closing`, `/api/trade`, `/api/rankings` (legacy), `/api/league-tendencies`. **Sim family (deterministic, no LLM):** `/api/sim/opponents` (GHOST DRAFT), `/api/odds` (Monte-Carlo title odds), `/api/spy` (opponent dossier + predicted picks), `/api/wheel-plan` (two-pick pair planner). Port **5001**. TTL cache. |
